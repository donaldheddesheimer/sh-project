...
export function TrendChart({ title, unit, points, markers, format = (v) => Math.round(v).toLocaleString('en-US') }: Props) {
  ...
  useEffect(() => {
    ...
    if (!el) return
    const observer = new ResizeObserver(([entry]) => setWidth(Math.max(80, entry.contentRect.width)))
    observer.observe(el)
  ...
  const yMax = niceMax(Math.max(...points.map((p) => p.v)) * 1.1)
  ...
  return (
    <div className="trend" ref={wrap}>
      ...
      {hovered && (
        <div className="tooltip" style={{ left: Math.min(width - 120, x(hovered.t) + 8), top: 24 }}>

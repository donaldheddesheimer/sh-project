...
- **Providers are interfaces.** `CityService` sees `SmartCityProvider` and `AgentProvider`
  ...
  Episode analysts are selected in the console from the configured teams; auto falls back to Mock
  without model credentials. A selected Nemotron team without `NVIDIA_API_KEY` would fail
